...
namespace WeChat.Tencent
{
    public class WXBizMsgCrypt
    {
        ...
        public WXBizMsgCrypt(string sToken, string sEncodingAESKey, string wxId, bool isCorp)
        {
            m_sToken = sToken;
            m_sCorpID = wxId;
        ...
        public bool CheckSignature(string signature, string token, string timestamp, string nonce)
        {
            string[] infos = new string[] { token, timestamp, nonce };
            Array.Sort<string>(infos);
            string info = string.Join("", infos);
            return string.Compare(signature, GetSha1Hash(info, Encoding.ASCII), true) == 0;
        }

        private string GetSha1Hash(string input, Encoding encoding)
        {
            StringBuilder sb = new StringBuilder();
            if (!string.IsNullOrEmpty(input) && encoding != null)
            {
